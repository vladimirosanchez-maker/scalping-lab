...
export default {
  async fetch(request, env, ctx) {
    ...
    const cooldownKey = new Request(new URL('/cache/binance/upstream-cooldown', url.origin));
    try {
      ...
      const cacheKey = new Request(new URL(`/cache/binance/market?symbol=${symbol}`, url.origin));
      const saved = await cache.match(cacheKey);
      if (saved) return new Response(saved.body, { headers });
      const contractKey = new Request(new URL(`/cache/binance/contract/${symbol}`, url.origin));
      ...
      const results = await Promise.allSettled([
        ...
        cachedContract ? cachedContract.json() : binance('/fapi/v1/exchangeInfo'),
      ]);
      const candles = {};
      ...
      const payload = { error: error.retryAt ? 'Binance ha limitado temporalmente las consultas. La conexión se recuperará automáticamente al terminar la pausa.' : `No se pudo actualizar el mercado. ${error.message}`, retryAt };
